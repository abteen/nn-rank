...
for model in xlm-roberta-base bert-base-multilingual-cased; do
    for target_split in dev_all; do
            for source_split in train_large train_medium train_all; do
                    ...
                    sbatch -J bible-pos-${model}-nn-${target_split} -o jobs/slurm_outputs/bible_results/pos/source_bible_target_bible/${model}/${source_split}_${target_split}/S-%x-%j.out \
                    --array 0-9 -N1 jobs/bible_pos_embedding_nn_efficient.slurm
            done
    done
done
