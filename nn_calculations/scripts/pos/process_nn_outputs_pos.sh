...
experiment=ud_pos_nn_sized
# max_k=25 # Represents the total number of neighbors calculated in previous step
    ...
    for method in simple_global_neighbor_distribution; do
        for layer in layer_8; do
            for train_split in train_all train_large train_medium; do
                for eval_split in dev_all dev_large dev_medium test_all test_large test_medium; do 
                    for k in 5 10 20 25; do

                        echo "currently working on ${train_split} ${eval_split} ${k}"

                        root_load_dir=/scratch/alpine/abeb4417/multilingual_analysis/ud_nn_outputs/${experiment}/${model}/${layer}/${train_split}/${eval_split}/
                        i=0
                        ...
                        for target_lang_dir in "${root_load_dir}"/*; do 
                            ...
                            python process_nn_outputs.py \
                                ...
                                --method ${method} \
                                --k ${k} \
                                --save_dir outputs/${experiment}/${model}/${train_split}/${eval_split}/${method}/${layer}/k_${k}/
