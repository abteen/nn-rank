...
def simple_global_neighbors_by_line_verbose(loaded_data, k):
    """ 
    ...
    print(f'Loaded {len(all_line_indices)} lines for the target language.')

    for line_idx in all_line_indices:

        print('*' * 10 + f' Processing line: {line_idx} ' + '*' * 10)
        ...
        print(f'Surface forms: {tokenizer.convert_ids_to_tokens(target_input_ids)}')

        # For each token, get their nearest neighbors
        ...
        for i in range(target_token_ids.shape[0]):
            ...
            print('\t'+f'Current token id: {target_token_id} token input id: {target_token_input_id} surface form: {tokenizer.convert_ids_to_tokens([target_token_input_id])}')

            neighbors = loaded_data['neighbors_indices'][target_token_id][:k]
            print('\t'+f'Neighbor ids: {neighbors}')

            neighbor_input_ids = loaded_data['source_input_ids'][neighbors]
            print('\t'+f'Neighbor input ids: {neighbor_input_ids}')
            print('\t'+f'Neighbor surface forms: {tokenizer.convert_ids_to_tokens(neighbor_input_ids)}')
            print('\t'+f"Neighbor source datasets: {loaded_data['source_langs'][neighbors]}")
...
if __name__ == '__main__':
    ...
    method_factory = {
        ...
        'sgnd-verbose' : simple_global_neighbors_by_line_verbose,
