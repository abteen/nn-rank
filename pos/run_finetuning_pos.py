...
from ma_utils.metrics import pos_accuracy_metric
...
if __name__ == '__main__':
    ...
    dev_file_to_load = args.dev_file

    if train_file_to_load is None:
        logging.error('train file did not load, skipping')
        quit()

    if dev_file_to_load is None:
        logging.error('did not load dev file')

    train_dataset = ConllPOSDataset(file=train_file_to_load, tokenizer=tokenizer)
    dev_dataset = ConllPOSDataset(file=dev_file_to_load, tokenizer=tokenizer) if dev_file_to_load else None
